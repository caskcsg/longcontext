...
model=${MODEL:-"your 128k base model path"}
# Point to the base dir of the ProLong 64K data
dataset=${DATASET:-"your 128k sft data path"}

dataset_name=${DATASET_NAME:-"ultrachat"}
# Directories in the dataset root folder where @ is followed by the mixing proportion 

domains=(
    ultrachat_200k_131072@1.0
)
...
save_steps=${SAVE:-500}
warmup=${WARMUP:-0.05}
suffix=${dataname:-"first70B_1000step"} # for model saving name

run_name="${suffix}_128k_sft"
