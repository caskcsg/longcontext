...
class DataArguments:
    ...
    disable_doc_mask: bool = field(default=False, metadata={"help": "Use doc mask"})
...
class DataCollator:
    ...
    def __call__(self, features):
        ...
        for item in features:
            ...
            if "position_weighted" in item:
                weighted_position = item["position_weighted"]
                if weighted_position is not None:
                    weighted_position = torch.tensor(weighted_position)
            ...
            for a, b in indices:
                ...
                if b - a > 1:
                    ...
                    if not self.args.disable_doc_mask:
        ...
        if weighted_positions:
            weighted_positions = torch.concat(weighted_positions, dim=0)
        else:
            weighted_positions = None
        if self.args.disable_doc_mask:
            return dict(input_ids=input_ids,
                    attention_mask=None,
                    labels=labels,
                    seq_lengths=torch.tensor([input_ids.size(0)], dtype=torch.long),
                    weighted_position=weighted_positions)
        else:
            return dict(input_ids=input_ids,
                        attention_mask=None,
                        labels=labels,
