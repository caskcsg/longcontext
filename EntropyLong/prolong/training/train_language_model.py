...
from training.modeling_flash_qwen2 import Qwen2ForCausalLM
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(script_args.tokenizer_name or script_args.model_name_or_path)
    ...
    if script_args.model_name_or_path:
        if 'llama' in script_args.model_name_or_path.lower():
            ...
            raise ValueError(f"Model {script_args.model_name_or_path} not supported")
    else:
        logger.warning(f"Initializing new LlamaForCausalLM from scratch")
        model = LlamaForCausalLM(config) if 'llama' in script_args.model_name_or_path else Qwen3ForCausalLM(config)
