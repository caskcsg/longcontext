...
class Trainer(HFTrainer):
    ...
    def get_sequence_parallel_inputs(self, inputs):
        ...
        if seq_parallel_world_size > 1:
            ...
            if "weighted_position" in inputs:
                weighted_position = inputs["weighted_position"]
                if weighted_position is not None:
                    if weighted_position[0] is not None:
                        weighted_position = torch.tensor(weighted_position, device=input_ids.device)
                    else:
            ...
            if weighted_position is not None:
                weighted_position = weighted_position.roll(-1, dims=-1)
            ...
            if weighted_position is not None:
                weighted_position_chunks = torch.tensor_split(weighted_position, seq_parallel_world_size)
            else:
                weighted_position_chunks = None
            ...
            inputs = {
                ...
                # "weighted_position": weighted_position_chunks[seq_parallel_rank] if weighted_position_chunks is not None else None,
            }
        ...
        **kwargs,
        ...
        if resume_from_checkpoint is not None:
            ...
            if os.path.isfile(os.path.join(resume_from_checkpoint, TRAINER_STATE_NAME)):
                state = TrainerState.load_from_json(os.path.join(resume_from_checkpoint, TRAINER_STATE_NAME)) # 读取state
