...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def compute_loss(self, hidden_states, labels, token_losses=False, weighted_position=None):
        self.t_step += 1
        ...
        if weighted_position is None:
            ...
            #     time.sleep(360000)
            losses = []
            # weighted_position和loss都是tensor，可以直接用torch.where实现加权 WEIGHT
            weight = int(os.getenv('WEIGHT', 5))
            weight_method = str(os.getenv('WEIGHT_METHOD', 'fixed'))
            if weight_method == 'fixed':
                losses = torch.where(weighted_position == 0, loss, loss * weight)
            elif weight_method == 'progressive':
                # 根据self.t_step从1逐渐增加到weight值，总step数为1000
                real_step = int(self.t_step / 64)
                # print(real_step)
                current_weight = 1 + (weight - 1) * min( real_step / 1000, 1.0)
                losses = torch.where(weighted_position == 0, loss, loss * current_weight)
            else:
                losses = torch.where(weighted_position == 0, loss, loss * weighted_position)
        ...
        if labels is not None or shifted_labels is not None:
            ...
            if self.logit_block_size > 0:
                ...
                if weighted_position is not None:
                    weighted_position = torch.split(weighted_position, self.logit_block_size, dim=0)
                else:
                    weighted_position = None
                if weighted_position is None:
                    weighted_position = [None] * len(labels)
                ...
                if getattr(self, "token_scaled_loss", False):
                    ...
                    loss = sum(
                        torch.utils.checkpoint.checkpoint(self.compute_loss,
                                                          ...
                                                          label_block,
                                                          use_reentrant=False,
                                                          weighted_position=weighted_position_block)
                        for hidden_state_block, label_block, weighted_position_block in zip(hidden_states, labels, weighted_position)
                    )
                else:
                    loss = sum(
                ...
                loss = self.compute_loss(hidden_states, labels, weighted_position)            
            logits = None
        else:
            logits = self.lm_head(hidden_states)
            loss = None
