...
if is_flash_attn_2_available():
    ...
    from flash_attn import flash_attn_func, flash_attn_varlen_func, flash_attn_kvpacked_func
...
def _flash_attention_forward(
    ...
    distributed_attn_func: Optional[Any] = None,
):
    """
    ...
    # Note: the `torch.diff(...)` condition is last to use short-circuit and avoid the cuda synchronization it incurs during inference (query_length == 1 always)
        ...
        if seq_parallel_group is not None and dist.is_initialized() and dist.get_world_size(seq_parallel_group) > 1:
            attention_func = distributed_attn_func
            flash_kwargs["group"] = seq_parallel_group
        else:
            attention_func = flash_attn_kvpacked_func
    
        kv_states = torch.stack([key_states, value_states], -3)

        # # import torch.distributed as dist
        # if dist.get_rank() == 0:
        #     import ipdb; ipdb.set_trace()
        #     print("-----", query_states.shape, kv_states.shape)
        # else:
        #     import time
        #     print("-----", query_states.shape, kv_states.shape)
        #     # time.sleep(1000)
        attn_output = attention_func(
            query_states, kv_states, dropout, softmax_scale=softmax_scale, causal=causal, **flash_kwargs
        )
